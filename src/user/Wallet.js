...
const Wallet = () => {
  ...
  async function getHistories() {
    try {
      // Use the new Ledger API
      const response = await walletAPI.getLedger({});
      const res = await response.json();
      if (res.success) {
        // According to user request: "data": { "transactions": [...] }
        const transactions = res.data?.transactions || [];
        // Optional: Manual sort if API doesn't guarantee it, though ledger usually does.
        // Keeping sort by date descending just in case.
